//
// - SecKC ESP8266 Captive Portal Hacking Challenge
...
IPAddress ip ( 1, 3, 3, 7 );                                                 // Private network for httpd
...
int rrsession;                                                                  // Connect Count Session
int rrtotal;                                                                    // Connect Count Total
char str_vcc[8];
int chan_selected;
...
void setupHTTPServer()
{
    ...
    httpd.on ( "/seckc", HTTP_GET, [] ( AsyncWebServerRequest * request )
    {
    ...
    httpd.on ( "/trigger", HTTP_GET, [] ( AsyncWebServerRequest * request )
    {
        ...
        printfAll ( "[[b;yellow;]Client Captured!] (%d): [%s] %s" ,
                       rrsession,
                       remoteIP.c_str(),
                       request->header ( "User-Agent" ).c_str()
                     );
        Serial.printf ( "Client Captured! (%d): [%s] %s\n",
                        rrsession,
                        remoteIP.c_str(),
                        request->header ( "User-Agent" ).c_str()
                      );
...
void onRequest ( AsyncWebServerRequest *request )
{
    ...
    if ( ( !SPIFFS.exists ( path ) && !SPIFFS.exists ( path + ".gz" ) ) ||  ( request->host() != "1.3.3.7" ) )
    {
...
//        response->setContentLength (CONTENT_LENGTH_UNKNOWN);
        response->addHeader ( "Location", "http://1.3.3.7/index.htm" );
        request->send ( response );

        /*        AsyncWebServerResponse *response = request->beginResponse(
                    511,
                    "text/html",
                    "<html><head><meta http-equiv='refresh' content='0; url=http://1.3.3.7/index.htm'></head></html>"
                );
...
void execCommand ( AsyncWebSocketClient *client, char *msg )
{
    ...
    else if ( !strncasecmp_P ( msg, PSTR ( "count" ), 5 ) )
    {
        ...
        client->printf_P ( PSTR ( "[[b;yellow;]Client Capture]: %d Session, %d Total" ), rrsession, rrtotal );
    }
